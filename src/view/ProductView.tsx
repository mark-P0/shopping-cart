...
import { Empty, formatPrice } from "../utilities.ts";
...
function ModalOverlay(props: PropsWithChildren<Empty>) {
  ...
  return (
    <div
      ref={ref}
      onClick={dismiss}
      className="fixed top-0 left-0 h-screen w-screen bg-black/50 grid place-items-center"
    >
      {children}
    </div>
  );
...
export function ProductView() {
  ...
  // console.log({ vehicles, productId, data });
    ...
    <ModalOverlay>
      <article className="overflow-hidden w-3/5 h-3/5 grid grid-cols-[3fr_2fr] bg-neutral-700 rounded-xl">
        ...
        <section className="overflow-hidden grid grid-rows-[auto_1fr_auto] gap-4 pr-4 py-6 pb-5">
          ...
          <p className="overflow-hidden overflow-y-auto scrollbar-thin scrollbar-track-neutral-500 scrollbar-thumb-neutral-600 bg-neutral-800 py-2 px-3">
            {description}
          </p>
